...
type Server struct {
	...
	// Adopting says the token was generated by the daemon rather than given to
	// it. The harness reads its environment when it launches, so the editor
	// that started this daemon may predate the value being written there, and
	// a daemon that enforced immediately would turn away every hook of the
	// session that started it while reporting itself healthy.
	//
	// It ends the moment the harness proves it has the token: one correct
	// header, and every later request must carry it. That is the whole window,
	// and it is bounded by the first hook of the first session rather than by
	// a timer, because a timer would either be too short to cover a slow start
	// or long enough to matter.
	Adopting bool
...
func (s *Server) authorised(r *http.Request) bool {
	...
	if s.Adopting && !s.adopted.Load() {
		s.saidAdopt.Do(func() {
			if s.Log != nil {
				s.Log.Info("accepting hooks without a token until the harness has picked up the generated one",
					"hint", "the value is in the harness settings already; it takes effect when the editor next starts")
			}
		})
		s.Metrics.Inc("shoulder_token_adopting_total")
		return true
	}
	return false
}
